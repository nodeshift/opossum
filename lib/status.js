  ...
  constructor (options) {
    ...
    if(options.distributed){
      ...
      this.distributed.client.connect()
          .then( res => this.redisClient = res )
          .catch(e => throw new Error("Redis Connection failed in opossum" + e))

      setInterval(() => {
        if(this.redisClient){

          //get all remaining keys
          this.redisClient.lrange(this.distributed.statsKey, this.distributed.lastReadIndex, -1)
              .then(res => {

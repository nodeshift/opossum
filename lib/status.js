  ...
  constructor (options) {
    ...
    if (options.distributed) {
      ...
      this.distributed.client.connect()
        .then(res => {
          this.redisClient = res;
        })
        .catch(e => {
          throw new Error(`Redis Connection failed in opossum ${e}`);
        });
        ...
        if (this.redisClient) {
          ...
          this.redisClient
            ...
            .then(res => {
              ...
              // add this as a new bucket in the window. Perform all side affects corresponding to it including removing last window
              //update lastReadIndex

              /*
              Performance: LRANGE performs linearly if either start is close to head or tail. In this case, we will have lastReadIndex close to tail as we are
...
const nextBucket = window => _ => {
  ...
  if (this.redisClient) {
    ...
    this.redisClient.lpush(this.distributed.statsKey, JSON.stringify(
      {
        nodeId: this.nodeId,
        stats: JSON.stringify(lastBucket)
      }
    ));
  }

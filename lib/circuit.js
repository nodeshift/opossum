...
const dbgCore = util.debuglog('dbg-core');
const dbgCache = util.debuglog('dbg-cache');
const dbgSemaphore = util.debuglog('dbg-semaphore');
...
class CircuitBreaker extends EventEmitter {
  ...
  constructor (action, options = {}) {
    super();
    dbgCore('inside the constructor');
    this.options = options;
    this.options.timeout =
      options.timeout === false ? false : options.timeout || 10000;
    dbgCore(`sets timeout to ${options.timeout}`);
    this.options.resetTimeout = options.resetTimeout || 30000;
    dbgCore(`sets resetTimeout to ${options.resetTimeout}`);
    ...
    dbgCore(`creates new semaphore with capacity ${this.options.capacity}`);
    ...
    function _startTimer (circuit) {
      return _ => {
        const timer = circuit[RESET_TIMEOUT] = setTimeout(() => {
          ...
          dbgCore('halfOpen event emitted');
  ...
  shutdown () {
     ...
     */
    this.emit('shutdown');
    dbgCore('shutdown event emitted');
  }
   ...
   */
  fire (...args) {
    dbgCore(`called fire function with args ${args}`);
  ...
  call (context, ...rest) {
    if (this.isShutdown) {
      ...
      dbgCore('rejected with shutdown error');
    ...
    if (CACHE.get(this) !== undefined) {
       ...
       */
      this.emit('cacheHit');
      dbgCache('cache hit event emitted');
       ...
       */
      this.emit('cacheMiss');
      dbgCache('cache miss event emitted');
    }

    if (!this[ENABLED]) {
      dbgCore('circuit is enabled');
    ...
    if (!this.closed && !this.pendingClose) {
      ...
      dbgCore('rejected event emitted, circuit is open');
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.semaphore.test()) {
        if (this.options.timeout) {
          timeout = setTimeout(
            () => {
              ...
              dbgSemaphore('semaphore released');
              /**
        ...
        dbgCore('semaphoreLocked event emitted');
  ...
  healthCheck (func, interval) {
    ...
    const check = _ => {
      func.apply(this).catch(e => {
        ...
        dbgCore('healthCheckFailed event emitted');
        this.open();
      });
    };
...
function fallback (circuit, err, args) {
  if (circuit[FALLBACK_FUNCTION]) {
    ...
    dbgCore('fallback event emitted');
...
function fail (circuit, err, args, latency) {
  ...
  dbgCore('failure event emitted');
  if (circuit.warmUp) return;

  // check stats to see if the circuit should be opened
  const stats = circuit.stats;
  dbgCore(`stats fires ${stats.fires}`);
  dbgCore(`circuit volume threshold  ${circuit.volumeThreshold}`);
  ...
  if (errorRate > circuit.options.errorThresholdPercentage ||
    ...
    circuit.halfOpen) {
    dbgCore(`error rate ${errorRate}`);
    dbgCore(`error threshold percentage ${circuit.options.errorThresholdPercentage}`);
    dbgCore(`stats failures ${stats.failures}`);
    dbgCore(`max failures ${circuit.options.maxFailures}`);
    dbgCore(`is half open ${circuit.halfOpen}`);
    circuit.open();
  }
}

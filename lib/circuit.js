...
class CircuitBreaker extends EventEmitter {
  ...
  */
  ...
  constructor (action, options = {}) {
    ...
    this.options.timeoutThresholdPercentage = 
      options.timeoutThresholdPercentage || 20; 
...
function fail (circuit, err, args, latency) {
  ...
  if (err.code === 'ECONNABORTED') {
    circuit.emit('timeout', err, latency, args);
  }
  ...
  const timeoutRate = stats.timeouts / stats.fires * 100;

  if (errorRate > circuit.options.errorThresholdPercentage ||
    circuit.halfOpen) {
    circuit.open();
  }
  if (timeoutRate > circuit.options.timeoutThresholdPercentage ||
    circuit.halfOpen) {
    circuit.open();
  }
}

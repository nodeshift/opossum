...
function normalizePrefix (prefixName) {
  ...
  prefixName = prefixName.replace(/-/g, '_');
  if (/^\d+$/.test(prefixName.charAt(0))) {
    prefixName = `a${prefixName.substring(1)}`;
  }
  return `${prefixName}${Date.now()}${i++}_`;
}
...
class PrometheusMetrics {
  constructor (circuit) {
    ...
    this._client = client;
    this.counters = [];
    const prefix = normalizePrefix(`${this.circuit.name}_`);
    ...
    for (let eventName of this.circuit.eventNames()) {
      const counter = new this._client.Counter({
        name: `${prefix}circuit_${eventName}`,

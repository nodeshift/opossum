...
test('Using cache', (t) => {
  ...
  breaker.fire(expected)
    .then((arg) => t.equals(arg, expected, `cache hit:misses ${breaker.status.cacheHit}:${breaker.status.cacheMiss}`))
    .catch(t.fail)
    .then(() => {
      breaker.fire(expected)
        .then((arg) => {
          t.equals(arg, expected, `cache hit:misses ${breaker.status.cacheHit}:${breaker.status.cacheMiss}`);
          breaker.clearCache();
        })
        .catch(t.fail)
        .then(() => {
